/*      File: gpio_lib.c
        Decription: Provides a mock up of an API to control GPIO pins from Linux userspace for
        the TMP coding challenge
...
/* Local Variables */
/* For this mock up library, the pin values and the direction are stored in simple arrays static to this
file */
...
int32_t gpio_set_direction(uint8_t port_id, uint8_t pin_id, uint8_t dir) {
    ...
    if(dir == DIR_OUTPUT) gpio_port_dirs[port_id] &= ~(1<<pin_id);
    else if (dir == DIR_INPUT) gpio_port_dirs[port_id] |= (1<<pin_id);
    else {
...
int32_t gpio_read_pin(uint8_t port_id, uint8_t pin_id, uint8_t* p_pin_state) {
    ...
    if ((gpio_port_pins[port_id] & (1<<pin_id))) *p_pin_state = 1;
    else *p_pin_state = 0;

    printf("Info: successfully read pin %d of port %d with a value of %d\r\n",
            pin_id, port_id, *p_pin_state);
...
int32_t gpio_write_pin(uint8_t port_id, uint8_t pin_id, uint8_t pin_state) {
    ...
    if(pin_state == 0) gpio_port_pins[port_id] &= ~(1<<pin_id);
    else if(pin_state == 1) gpio_port_pins[port_id] |= (1<<pin_id);
    else {
